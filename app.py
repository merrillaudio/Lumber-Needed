...
def generate_required_pieces(required_df):
    pieces = []
    job_column_exists = 'Job' in required_df.columns
    for _, row in required_df.iterrows():
        ...
        for _ in range(quantity):
            ...
            if job_column_exists:
                piece['job'] = row.get('Job', '')
            pieces.append(piece)
...
def generate_pdf(purchased_boards, leftovers=None):
    ...
    with PdfPages(buffer) as pdf:
        ...
        for board in purchased_boards:
            ...
            ax_text.axis('off')
            text_lines = []
            header = f"{'Piece ID':<12} {'X':>6} {'Y':>6} {'L':>8} {'W':>8} {'Rotated':>8} {'Job':>10}"
            ...
            for cut in board['cuts']:
                ...
                job_str = piece.get('job', '')
                line = f"{piece_id:<12} {x_str:>6} {y_str:>6} {length_str:>8} {width_str:>8} {rotated_str:>8} {job_str:>10}"
        ...
        text_lines = []
        text_lines.append(f"{'Board Dimensions':<20} {'Quantity':>10} {'Jobs':<30}")
        ...
        for dims, qty in sorted(summary_data.items()):
            ...
            job_labels = []
            for board in purchased_boards:
                if (board['board']['length_ft'], board['board']['width_in']) == dims:
                    for cut in board['cuts']:
                        job = cut['piece'].get('job')
                        if job and job not in job_labels:
                            job_labels.append(job)
            job_str = ", ".join(job_labels)
            line = f"{dims_str:<20} {qty:>10} {job_str:<30}"
            text_lines.append(line)
        text = "\n".join(text_lines)
...
def default_cut_df():
    return pd.DataFrame([{"Length": "24", "Width": "6", "Quantity": 2, "Job": ""}])
existing_jobs = set()
if 'required_df' in st.session_state:
    existing_jobs = set(st.session_state['required_df'].get('Job', '').dropna().unique())

required_df = st.data_editor(
    st.session_state.get('required_df', default_cut_df()),
    column_config={
        "Job": st.column_config.SelectboxColumn("Job", options=sorted(existing_jobs))
    },
    column_order=["Length", "Width", "Quantity", "Job"],
    num_rows="dynamic", use_container_width=True,
    key="required_cut_table"
)
